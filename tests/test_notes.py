...
def test_create_source_note_includes_house_style_sections(isolated_workspace: Path) -> None:
    ...
    assert 'description: "SEP source note for Sample Entry and its role in the current cluster."' in note_text
...
def test_create_person_page_defaults_title_from_slug(isolated_workspace: Path) -> None:
    ...
    assert 'description: "Seed person page for Thomas Aquinas and their relevance to the current cluster."' in person_text
